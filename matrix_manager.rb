...
class MatrixManager
  ...
  def initialize(start_col, start_row, zoom, offset_x, offset_y, tile_mgr)
    @tile_mgr = tile_mgr
    @zoom = zoom
    @tile_col = start_col
    @tile_row = start_row
      ...
      if new_width > @matrix.width
        (@tile_col + @matrix.width..@tile_col + new_width - 1).each do |col|
          ...
          (@tile_row..@tile_row + @matrix.height - 1).each do |row|
            column << @tile_mgr.get_tile(col, row, @zoom)
          end
      ...
      if new_height > @matrix.height
        (@tile_row + @matrix.height..@tile_row + new_height - 1).each do |r|
          ...
          (@tile_col..@tile_col + new_width - 1).each do |col|
            row << @tile_mgr.get_tile(col, r, @zoom)
          end
          @matrix.add_row(row)
        end
      end
  ...
  def initialize_matrix
    ...
    @matrix[0,0] = @tile_mgr.get_tile(@tile_col, @tile_row, @zoom)
  end    
  ...
  def get_tiles(what, which)
    ...
    (0..size).map do |index|
      @tile_mgr.get_tile(col + (what == :row ? index : 0),
